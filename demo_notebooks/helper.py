...
def plot(params,X,Y,LLR = None,test_name ='Infer_trajectory',sparse = False):
    ...
    for i in range(len(indexes)):
        ...
        print(indexes[i])
        model_index = 1
        y = Y.iloc[i] # genes expression data
        p = {}
        ...
        for mean,var,model in zip(params['means'][i],params['vars'][i],params['models'][i]):
            ...
            llr = list(chain(*LLR.loc[indexes[[i]]].values))
           
            if sparse:
                Z = model.inducing_variable.Z.numpy()
            else:
                Z = None
            plot_GP(p,X, y, llr,sparse = sparse,Z = Z,model_index = model_index,test_name = params['test_name'],gene_name = indexes[i])
            model_index = model_index + 1
            
def plot_GP(params,X,y,LLR = None,sparse = False,Z = None, model_index = 1, test_name = None,gene_name = 'GP'):
    ...
    if not(test_name == 'Two_samples_test' and model_index == 3):
        fig = plt.figure()
    ...
    if test_name == 'Two_samples_test':
        if model_index == 1:
            if LLR is not None:
                label = LLR[3]
                plt.scatter(X[0:int(X.shape[0]/2)],y[0:int(X.shape[0]/2)], s=30, marker='o', color= 'royalblue',alpha=1.,label = str(gene_name)+'-LLR='+'{00:.2f}'.format(label)) #data               
                plt.legend(loc='best') 
            else:
        ...
        if LLR is not None:
            if test_name == 'One_sample_test':
                label =  str(gene_name)+'-LLR ='+'{00:.2f}'.format(LLR[2])
                ...
                if test_name == None:
                    ...
                    llr = LLR[0]
                label = str(gene_name)+'-LL ='+'{00:.2f}'.format(llr)
